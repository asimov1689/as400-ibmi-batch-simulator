...
public class PortfolioRepository {
    ...
    public Optional<Portfolio> findById(String portfolioId) {
        String sql = "SELECT PORTF_ID, OWNER, CURRENCY, TOTAL_VALUE, STATUS, LAST_UPD " +
                "FROM " + table("PORTFOLIO") + " WHERE PORTF_ID = ?";

        try {
            List<Portfolio> rows = jdbc.query(sql, portfolioMapper,
    ...
    public List<TradeOrder> findPendingOrders() {
        String sql = "SELECT ORDER_ID, PORTF_ID, ISIN, QUANTITY, PRICE, STATUS " +
